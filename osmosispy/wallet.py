...
BECH32_PUBKEY_CONS_PREFIX = "osmovalconspub"

BECH32_ADDR_ACC_PREFIX = "osmo"
BECH32_ADDR_VAL_PREFIX = "osmovaloper"
BECH32_ADDR_CONS_PREFIX = "osmovalcons"
...
class PrivateKey:
    ...
    def from_hex(cls, priv: str) -> "PrivateKey":
        self = cls(_error_do_not_use_init_directly=True)
        self.signing_key = SigningKey.from_string(
            bytes.fromhex(priv), curve=SECP256k1)
        return self
...
class PublicKey:
    ...
    def __init__(self, _error_do_not_use_init_directly=None) -> None:
        ...
        if not _error_do_not_use_init_directly:
            raise TypeError(
                "Please use PublicKey's factory methods to construct me")
    ...
    def _to_bech32(self, prefix: str) -> str:
        five_bit_r = convertbits(
            ...
            bytes.fromhex("eb5ae98721") + \
            self.verify_key.to_string("compressed"),
            8,
            5,
        )
